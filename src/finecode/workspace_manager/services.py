...
from finecode.workspace_manager.runner import manager as runner_manager, runner_info
...
async def restart_extension_runners(
    ...
    # TODO: reload config?
    try:
        runners_by_env = ws_context.ws_projects_extension_runners[runner_working_dir_path]
    except KeyError:
    ...
    for runner in runners_by_env.values():
        ...
        new_runner = await runner_manager.start_extension_runner(
            runner_dir=runner_working_dir_path, env_name=runner.env_name, ws_context=ws_context
        )
    ...
    ws_context.ws_projects_extension_runners[runner_working_dir_path] = new_runners_by_env
...
async def run_action(
    ...
    if project_def.status != domain.ProjectStatus.CONFIG_VALID:
        raise ActionRunFailed(f"Project {project_def.dir_path} has no valid configuration and finecode."
            " Please check logs.")
    ...
    action = next(action for action in project_def.actions if action.name == action_name)
...
async def _run_action_in_env_runner(
    ...
    if runner.status != runner_info.RunnerStatus.RUNNING:
        raise ActionRunFailed(f"Runner {env_name} in project {project_def.dir_path} is not running. Status: {runner.status}")
    ...
    try:
        ...
        raise ActionRunFailed(f"Action {action_name} failed: {error.message}")
