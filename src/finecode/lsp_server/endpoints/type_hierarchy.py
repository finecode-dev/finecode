...
def _lsp_item_to_action(lsp_item: dict[str, Any]) -> dict[str, Any]:
    """Convert an LSP TypeHierarchyItem (camelCase) to action payload form (snake_case)."""
    return {
        "name": lsp_item["name"],
        "kind": lsp_item["kind"],
        "uri": lsp_item["uri"],
...
def _action_item_to_lsp(item: dict[str, Any]) -> dict[str, Any]:
    """Convert an action-result TypeHierarchyItem (snake_case) to LSP wire form (camelCase)."""
    result: dict[str, Any] = {
        "name": item["name"],
        "kind": item["kind"],
        "uri": item["uri"],
...
async def type_hierarchy_supertypes(
    ...
    try:
        response = await global_state.wm_client.run_action(
            action_source="fine_code_hierarchy.TypeHierarchySupertypesAction",
            project=project_dir,
...
async def type_hierarchy_subtypes(
    ...
    try:
        response = await global_state.wm_client.run_action(
            action_source="fine_code_hierarchy.TypeHierarchySubtypesAction",
            project=project_dir,
            params={"item": _lsp_item_to_action(lsp_item)},
            options={"trigger": "user", "devEnv": "ide"},
        )
