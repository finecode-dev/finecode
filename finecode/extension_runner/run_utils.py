...
def get_project_venv_path(project_path: Path) -> Path:
    ...
    if venv_path_match is None:
        logger.error("Venv path not found in poetry output")
        raise VenvNotFound()

    project_venv_path_str = venv_path_match.group("venv_path")
    if project_venv_path_str == "NA":
        # it can be checked whether venv exists with `poetry env list` and then either
        # automatically activated or suggested to user to activate
        logger.error(
            f"No virtualenv found in {project_path}."
            " Maybe it is not activated or you have wrong default env version?"
        )
        raise VenvNotFound()
...
def run_cmd_in_dir(cmd: str, dir_path: Path) -> tuple[int, str]:
    ...
    os.chdir(dir_path)
    # remove 'VIRTUAL_ENV' env variable to avoid impact of current venv if one is
    # activated
