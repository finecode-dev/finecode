...
def find_package_for_file(file_path: Path, workspace_path: Path) -> Path:
    ...
    while current_path != workspace_path:
        ...
        if pyproject_path.exists() and _finecode_is_enabled_in_def(def_file=pyproject_path):
            return current_path
...
def find_package_with_action_for_file(
    ...
    logger.trace(f"Find package with action {action_name} for file {file_path.as_posix()}")
    ...
    if (
        ws_context.package_path_by_dir_and_action.get(dir_path_str, {}).get(action_name, None)
        is not None
    ):
        logger.trace(
    ...
    while current_path != workspace_path:
        ...
        if pyproject_path.exists() and _finecode_is_enabled_in_def(def_file=pyproject_path):
            ...
            try:
                ...
                ws_context.package_path_by_dir_and_action[dir_path_str][action_name] = current_path
                return current_path
