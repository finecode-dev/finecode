...
async def run_actions(workdir_path: pathlib.Path, projects_names: list[str] | None, actions: list[str], action_payload: dict[str, str], concurrently: bool) -> tuple[str, int]:
    ws_context = context.WorkspaceContext([workdir_path])
    await read_configs.read_projects_in_dir(dir_path=workdir_path, ws_context=ws_context)
    ...
    try:
        try:
            ...
            raise RunFailed(f"One or more projects are misconfigured, runners for them didn't start. Check logs for details.")

        actions_by_projects: dict[pathlib.Path, list[str]] = {}
        if projects_names is not None:
            projects: list[domain.Project] = get_projects_by_names(projects_names, ws_context, workdir_path)
            ...
            for project in projects:
                project_actions_set: ordered_set.OrderedSet[str] = ordered_set.OrderedSet([action.name for action in project.actions])
                missing_actions = actions_set - project_actions_set
                if len(missing_actions) > 0:
                    raise RunFailed(f"Actions {', '.join(missing_actions)} not found in project '{project.name}'")
        ...
        return await run_actions_in_all_projects(actions_by_projects, action_payload, ws_context, concurrently)
    finally:
        services.on_shutdown(ws_context)


def get_projects_by_names(projects_names: list[str], ws_context: context.WorkspaceContext, workdir_path: pathlib.Path) -> list[domain.Project]:
    projects: list[domain.Project] = []
    for project_name in projects_names:
        try:
            project = next(project for project in ws_context.ws_projects.values() if project.name == project_name)
        except StopIteration:
            raise RunFailed(f"Project '{projects_names}' not found in working directory '{workdir_path}'")

        projects.append(project)
    return projects


def find_projects_with_actions(ws_context: context.WorkspaceContext, actions: list[str]) -> dict[pathlib.Path, list[str]]:
    ...
    for project in ws_context.ws_projects.values():
        ...
        action_to_run_in_project = ordered_set.OrderedSet(project_actions_names) - actions_set
        actions_by_project[project.dir_path] = list(action_to_run_in_project)

    return actions_by_project


def run_result_to_str(run_result: str | dict[str, list[str | dict[str, str | bool]]], action_name: str) -> str:
    run_result_str = ''
    if isinstance(run_result, str):
        ...
        if not isinstance(text_parts, list):
            raise RunFailed(f"Running of action {action_name} failed: got unexpected result, 'parts' value expected to be a list.")
        ...
        for text_part in text_parts:
            if isinstance(text_part, str):
                ...
                try:
                    ...
                    raise RunFailed(f"Running of action {action_name} failed: got unexpected result, 'text' value is required in object with styled text params.")
                
                style_params: dict[str, str | bool] = {}
                if 'foreground' in text_part and isinstance(text_part['foreground'], str):
                    style_params['fg'] = text_part['foreground']
                
                if 'background' in text_part and isinstance(text_part['background'], str):
                ...
                if 'underline' in text_part and isinstance(text_part['underline'], bool):
                ...
                if 'strikethrough' in text_part and isinstance(text_part['strikethrough'], bool):
                ...
                raise RunFailed(f"Running of action {action_name} failed: got unexpected result, 'parts' list can contain only strings or objects with styled text.")
...
async def run_actions_in_running_project(actions: list[str], action_payload: dict[str, str], project: domain.Project, ws_context: context.WorkspaceContext, concurrently: bool) -> dict[str, ActionRunResult]:
    ...
    if concurrently:
        ...
        try:
            async with asyncio.TaskGroup() as tg:
                for action_name in actions:
                    run_task = tg.create_task(services.run_action(action_name=action_name, params=action_payload, project_def=project, ws_context=ws_context, result_format=services.RunResultFormat.STRING))
        ...
        for idx, run_task in enumerate(run_tasks):
            ...
            result_by_action[action_name] = ActionRunResult(output=run_result_str, return_code=run_result.return_code)
    else:
        for idx, action_name in enumerate(actions):
            try:
                run_result = await services.run_action(action_name=action_name, params=action_payload, project_def=project, ws_context=ws_context, result_format=services.RunResultFormat.STRING)
...
async def run_actions_in_all_projects(actions_by_project: dict[pathlib.Path, list[str]], action_payload: dict[str, str], ws_context: context.WorkspaceContext, concurrently: bool) -> tuple[str, int]:
    ...
    try:
        async with asyncio.TaskGroup() as tg:
            for project_dir_path, actions_to_run in actions_by_project.items():
                ...
                project_task = tg.create_task(run_actions_in_running_project(actions=actions_to_run, action_payload=action_payload, project=project, ws_context=ws_context, concurrently=concurrently))
