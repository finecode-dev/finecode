...
def find_project_with_action_for_file(
    ...
    while current_path != workspace_path:
        pyproject_path = current_path / "pyproject.toml"
        if pyproject_path.exists() and _finecode_is_enabled_in_def(def_file=pyproject_path):
            actions = collect_actions(project_path=current_path, ws_context=ws_context)
            try:
                next(action for action in actions if action.name == action_name)
                ws_context.project_path_by_dir_and_action[dir_path_str][action_name] = current_path
                return current_path
            except StopIteration:
                ...
        current_path = current_path.parent

    ws_context.project_path_by_dir_and_action[dir_path_str][action_name] = workspace_path
    return workspace_path
