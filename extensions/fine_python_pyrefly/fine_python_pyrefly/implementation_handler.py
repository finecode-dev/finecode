...
class PyreflyImplementationHandlerConfig(code_action.ActionHandlerConfig):
    pass


class PyreflyImplementationHandler(
    ...
    def __init__(
        self,
        config: PyreflyImplementationHandlerConfig,
    ...
    async def run(
        ...
        result = await self.lsp_service.get_implementation(file_path, content, position)
        return ImplementationResult(locations=locations_from_lsp(result))
