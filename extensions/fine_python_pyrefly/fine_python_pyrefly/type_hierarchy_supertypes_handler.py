...
class PyreflyTypeHierarchySupertypesHandlerConfig(code_action.ActionHandlerConfig):
    pass


class PyreflyTypeHierarchySupertypesHandler(
    ...
    FILE_OPERATION_AUTHOR = ifileeditor.FileOperationAuthor(
        id="PyreflyTypeHierarchySupertypesHandler"
    )

    def __init__(
        self,
        config: PyreflyTypeHierarchySupertypesHandlerConfig,
    ...
    async def run(
        ...
        raw_result = await self.lsp_service.get_type_hierarchy_supertypes(
            file_path, content, item_dict
        )
        if not raw_result:
            return TypeHierarchySupertypesResult(items=[])

        return TypeHierarchySupertypesResult(
