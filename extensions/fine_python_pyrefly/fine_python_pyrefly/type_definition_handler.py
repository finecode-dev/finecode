...
from fine_symbol_info.text_document_type_definition_action import TypeDefinitionPayload, TypeDefinitionResult
from fine_python_lang.text_document_type_definition_python_action import TextDocumentTypeDefinitionPythonAction
...
class PyreflyTypeDefinitionHandlerConfig(code_action.ActionHandlerConfig):
    pass


class PyreflyTypeDefinitionHandler(
    code_action.ActionHandler[
        TextDocumentTypeDefinitionPythonAction,
        PyreflyTypeDefinitionHandlerConfig,
    ]
):
    FILE_OPERATION_AUTHOR = ifileeditor.FileOperationAuthor(id="PyreflyTypeDefinitionHandler")

    def __init__(
        self,
        config: PyreflyTypeDefinitionHandlerConfig,
    ...
    async def run(
        ...
        result = await self.lsp_service.get_type_definition(file_path, content, position)
        return TypeDefinitionResult(locations=locations_from_lsp(result))
