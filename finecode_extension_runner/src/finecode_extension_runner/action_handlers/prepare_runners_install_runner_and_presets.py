...
class PrepareRunnersInstallRunnerAndPresetsHandler(
    ...
    async def run(
        ...
        for env in envs:
            ...
            project_def_path = run_context.project_def_path_by_venv_dir_path[env.venv_dir_path]
            try:
                dependencies = get_dependencies_in_project_raw_config(project_def, env.name)
            except FailedToGetDependencies as exception:
                raise code_action.ActionFailedException(f"Failed to get dependencies of env {env.name} in {project_def_path} (install_runner_and_presets handler)")
...
def get_dependencies_in_project_raw_config(project_raw_config: dict[str, typing.Any], env_name: str):
    # returns dependencies: presets and extension runner
    presets_in_config = project_raw_config.get('tool', {}).get('finecode', {}).get('presets', [])
    ...
    env_deps_config = project_raw_config.get('tool', {}).get('finecode', {}).get('env', {}).get(env_name, {}).get('dependencies', {})
    dependencies = []
    
    try:
        runner_dep = next(dep for dep in env_raw_deps if isinstance(dep, str) and dependency_config_utils.get_dependency_name(dep) == 'finecode_extension_runner')
    except StopIteration:
        raise FailedToGetDependencies(f"prepare_runners expects finecode_extension_runner dependency in each environment, but it was not found in {env_name}")
    
    runner_dep_dict = dependency_config_utils.raw_dep_to_dep_dict(raw_dep=runner_dep, env_deps_config=env_deps_config)
    dependencies.append(runner_dep_dict)
    
    for preset_package in presets_packages_names:
        try:
            preset_dep = next(dep for dep in env_raw_deps if isinstance(dep, str) and dependency_config_utils.get_dependency_name(dep) == preset_package)
        except StopIteration:
            if env_name == 'dev_no_runtime':
                # all preset packages must be in 'dev_no_runtime' env
                raise FailedToGetDependencies(f"'{preset_package}' is used as preset source, but not declared in 'dev_no_runtime' dependency group")
            else:
                continue
        
        preset_dep_dict = dependency_config_utils.raw_dep_to_dep_dict(raw_dep=preset_dep, env_deps_config=env_deps_config)
