...
class TypeHierarchySupertypesDispatchHandlerConfig(code_action.ActionHandlerConfig): ...


class TypeHierarchySupertypesDispatchHandler(
    code_action.ActionHandler[
        TypeHierarchySupertypesAction,
        TypeHierarchySupertypesDispatchHandlerConfig,
    ]
):
    """Dispatch type_hierarchy_supertypes to language-specific subactions.
    ...
    (e.g. TypeHierarchySupertypesPythonAction). If no subaction is registered
    ...
    async def _dispatch_to_lang(
        ...
        payload: TypeHierarchySupertypesPayload,
    ...
    async def run(
        ...
        if not subactions_by_lang:
            self.logger.debug(
                "TypeHierarchySupertypesDispatchHandler: no language subactions registered"
            )
        ...
        if coverage:
            await run_context.partial_result_sender.send(
                TypeHierarchySupertypesResult(coverage=coverage)
            )
            return
