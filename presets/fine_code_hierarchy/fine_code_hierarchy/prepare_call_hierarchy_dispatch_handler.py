...
class PrepareCallHierarchyDispatchHandlerConfig(code_action.ActionHandlerConfig): ...


class PrepareCallHierarchyDispatchHandler(
    code_action.ActionHandler[
        TextDocumentPrepareCallHierarchyAction,
        PrepareCallHierarchyDispatchHandlerConfig,
    ]
):
    """Dispatch text_document_prepare_call_hierarchy to language-specific subactions.
    ...
    (e.g. TextDocumentPrepareCallHierarchyPythonAction). If no subaction is registered
    ...
    async def run(
        ...
        if not subactions_by_lang:
            self.logger.debug(
                "PrepareCallHierarchyDispatchHandler: no language subactions registered"
            )
        ...
        if coverage:
            await run_context.partial_result_sender.send(
                PrepareCallHierarchyResult(coverage=coverage)
            )
            return
