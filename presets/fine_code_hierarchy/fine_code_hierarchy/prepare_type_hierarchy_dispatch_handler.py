...
class PrepareTypeHierarchyDispatchHandlerConfig(code_action.ActionHandlerConfig): ...


class PrepareTypeHierarchyDispatchHandler(
    code_action.ActionHandler[
        TextDocumentPrepareTypeHierarchyAction,
        PrepareTypeHierarchyDispatchHandlerConfig,
    ]
):
    """Dispatch text_document_prepare_type_hierarchy to language-specific subactions.
    ...
    (e.g. TextDocumentPrepareTypeHierarchyPythonAction). If no subaction is registered
    ...
    async def run(
        ...
        if not subactions_by_lang:
            self.logger.debug(
                "PrepareTypeHierarchyDispatchHandler: no language subactions registered"
            )
        ...
        if coverage:
            await run_context.partial_result_sender.send(
                PrepareTypeHierarchyResult(coverage=coverage)
            )
            return
