...
class InlayHintDispatchHandlerConfig(code_action.ActionHandlerConfig): ...


class InlayHintDispatchHandler(
    code_action.ActionHandler[
        TextDocumentInlayHintAction,
        InlayHintDispatchHandlerConfig,
    ]
):
    """Dispatch text_document_inlay_hint to language-specific subactions.
    ...
    (e.g. TextDocumentInlayHintPythonAction). If no subaction is registered for
    the document's language, returns no result.
    """
    ...
    async def run(
        ...
        if not subactions_by_lang:
            self.logger.debug(
                "InlayHintDispatchHandler: no language subactions registered"
            )
        ...
        if coverage:
            await run_context.partial_result_sender.send(
                InlayHintResult(coverage=coverage)
            )
            return
