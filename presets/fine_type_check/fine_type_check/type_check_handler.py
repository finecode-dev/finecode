...
class TypeCheckHandler(
    code_action.ActionHandler[type_check_action.TypeCheckAction, TypeCheckHandlerConfig]
):
    def __init__(
        self,
    ...
    async def _type_check_project(
        ...
        if not results:
            ...
            await partial_result_sender.send(
                type_check_action.TypeCheckRunResult(
        ...
        for result in results.values():
            ...
            await partial_result_sender.send(
                type_check_action.TypeCheckRunResult(messages=result.messages)
            )
    ...
    async def run(
        ...
        if not file_uris:
            ...
            await run_context.partial_result_sender.send(
                type_check_action.TypeCheckRunResult(messages={})
            )
            return
        ...
        async with run_context.progress(
            "Type-checking files", total=len(file_uris)
        ) as progress:
            ...
            if unassigned_uris:
                await run_context.partial_result_sender.send(
                    type_check_action.TypeCheckRunResult(
                        messages={u: [] for u in unassigned_uris}
                    )
                )
