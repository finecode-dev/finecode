...
class InspectCodePrecommitBridgeHandlerConfig(code_action.ActionHandlerConfig): ...


class InspectCodePrecommitBridgeHandler(
    code_action.ActionHandler[
        precommit_action.PrecommitAction, InspectCodePrecommitBridgeHandlerConfig
    ]
):
    """Bridge handler that runs inspect_code on the staged files."""
    ...
    async def run(
        ...
        if not files_by_project:
            self.logger.warning(
                "Staged files do not belong to any workspace project - skipping inspect_code."
            )
            return precommit_action.PrecommitRunResult()
