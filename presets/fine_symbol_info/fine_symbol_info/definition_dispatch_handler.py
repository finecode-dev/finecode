...
class DefinitionDispatchHandlerConfig(code_action.ActionHandlerConfig): ...


class DefinitionDispatchHandler(
    code_action.ActionHandler[
        TextDocumentDefinitionAction,
        DefinitionDispatchHandlerConfig,
    ]
):
    """Dispatch text_document_definition to language-specific subactions.
    ...
    (e.g. TextDocumentDefinitionPythonAction). If no subaction is registered
    ...
    async def run(
        ...
        if not subactions_by_lang:
            self.logger.debug(
                "DefinitionDispatchHandler: no language subactions registered"
            )
        ...
        if coverage:
            await run_context.partial_result_sender.send(
                DefinitionResult(coverage=coverage)
            )
            return
