...
class DocumentHighlightDispatchHandlerConfig(code_action.ActionHandlerConfig): ...


class DocumentHighlightDispatchHandler(
    code_action.ActionHandler[
        TextDocumentDocumentHighlightAction,
        DocumentHighlightDispatchHandlerConfig,
    ]
):
    """Dispatch text_document_document_highlight to language-specific subactions.
    ...
    (e.g. TextDocumentDocumentHighlightPythonAction). If no subaction is registered
    ...
    async def run(
        ...
        if not subactions_by_lang:
            self.logger.debug(
                "DocumentHighlightDispatchHandler: no language subactions registered"
            )
        ...
        if coverage:
            await run_context.partial_result_sender.send(
                DocumentHighlightResult(coverage=coverage)
            )
            return
