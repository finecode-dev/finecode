...
class ReferencesDispatchHandlerConfig(code_action.ActionHandlerConfig): ...


class ReferencesDispatchHandler(
    code_action.ActionHandler[
        TextDocumentReferencesAction,
        ReferencesDispatchHandlerConfig,
    ]
):
    """Dispatch text_document_references to language-specific subactions.
    ...
    (e.g. TextDocumentReferencesPythonAction). If no subaction is registered
    ...
    async def run(
        ...
        if not subactions_by_lang:
            self.logger.debug(
                "ReferencesDispatchHandler: no language subactions registered"
            )
        ...
        if coverage:
            await run_context.partial_result_sender.send(
                ReferencesResult(coverage=coverage)
            )
            return
