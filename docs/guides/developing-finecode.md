...
**Only the root `dev_workspace` stays editable in CI.** It is the tool running the checks, not code under test, so `setup-dev-workspace.sh` installs it editable — but every other env is installed from wheels built once for the checkout. `finecode-workspace.toml` selects the mode per dev-env:
...
`prepare-envs` builds each workspace package's wheel with `build_python_artifact` — each on its own project's runner, so per-project builder overrides and config apply — into `<workspace-root>/.venvs/dev_workspace/cache/wheelhouse`, then installs every non-`dev_workspace` env from those wheels. A cold install is measured in minutes instead of the ~2h an all-editable install cost. Local dev-envs default to `editable`; `prepare-envs --workspace-packages=wheel|editable` overrides the mode for one run. The wheelhouse is a snapshot: an edit to a workspace package is not visible to a wheel-mode env until the next `prepare-envs`.

**uv cache placement in CI.** Every CI job that runs `prepare-envs` points `UV_CACHE_DIR` at `$RUNNER_TEMP/uv-cache`, on the same filesystem as the venvs, so uv can hardlink package files instead of copying them. `RUNNER_TEMP` is wiped between jobs, so the cache is persisted with `actions/cache/restore`/`save`, restored (never blocking) only when the venvs cache misses, and trimmed of workspace-package entries before it is saved. Each saved entry is a new multi-GB artifact, so four of them (three public OS legs plus the private job) compete for the repository's cache quota; if the total does not fit, the least-valuable leg is dropped. Full details and the local verification recipe: [`uv` cache placement in containers](preparing-environments.md#uv-cache-placement-in-containers).
...
**Knowledge-package coverage.** `finecode_knowledge` is a public, tracked package and a hard runtime dependency of `finecode`, so the `build` job's `run_tests` covers both `finecode`'s own `tests/unit/test_knowledge_service*.py` and `finecode_knowledge`'s tests on every platform. The private `fine_knowledge` schema — the entity types, providers, predicates, rules and the `extract_knowledge` / `which_handlers` / `audit_preset_deps` actions — is absent from a public checkout and is covered only by the `audit-private` job.

**Private-layer audit in CI.** The public CI pipeline runs a separate, non-required `audit-private` job that clones the two private repositories (`fine_knowledge` and `finecode_internal_experiments`) with an App token minted per run, copies `.github/ci/finecode-user.ci.toml` into place as the tracked counterpart to a developer's gitignored `finecode-user.toml`, and then runs `inspect_code`, `extract_knowledge`, `audit_code` and `run_tests` — the same check coverage the `build` job gives the public packages, so a private-repo regression in linting, type checking or tests is caught here too, not just a broken `preset_deps` audit. The job is gated so fork pull requests skip it and the remaining steps are gated on the App credentials being present; it is separate from the `build` job so a break in a private repository reds one non-required check instead of blocking every merge. `extract_knowledge` must precede `audit_code` because `AuditPresetDepsHandler` never re-extracts a missing fact store itself — without it the audit emits a diagnostic telling the operator to run `extract_knowledge` first. The `.ci` infix in the filename is load-bearing, not cosmetic: `.gitignore`'s unanchored `finecode-user.toml` pattern matches at any depth, so the plain name could not be tracked at all.
