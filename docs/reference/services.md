...
| `finecode_extension_api.interfaces.iworkspaceactionrunner.IWorkspaceActionRunner` | `finecode_extension_runner.impls.workspace_action_runner.WorkspaceActionRunnerImpl` | Fan-out an action across workspace projects — one payload everywhere (`run_action_in_projects`), or a complete payload per project (`run_action_per_project`). |
