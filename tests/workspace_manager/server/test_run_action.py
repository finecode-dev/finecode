...
async def test__runs_action_in_package(client_channel):
    ...
    request = RunActionRequest(action_node_id=f'{cli_tool_root_dir_path.as_posix()}::format', apply_on=unformatted_src_path.as_posix(), apply_on_text='')    
...
async def test__runs_general_action_in_package(client_channel):
    ...
    request = RunActionRequest(action_node_id='format', apply_on=unformatted_src_path.as_posix(), apply_on_text='')    
    response = await WorkspaceManagerService.run_action(channel=client_channel, request=request)
