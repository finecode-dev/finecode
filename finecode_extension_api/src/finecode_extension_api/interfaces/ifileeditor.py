...
class IFileEditorSession(Protocol):
    ...
    #   author files
    async def change_file(self, file_path: pathlib.Path, change: FileChange) -> None:
        ...

    @contextlib.asynccontextmanager
    async def subscribe_to_changes_of_opened_files(self) -> collections.abc.AsyncIterator[FileChangeEvent]:
    ...
    @contextlib.asynccontextmanager
    async def read_file(self, file_path: pathlib.Path, block: bool = False) -> collections.abc.AsyncIterator[FileInfo]:
        ...
...
class IFileEditor(Protocol):
    ...
    """
    @contextlib.asynccontextmanager
    async def session(self, author: FileOperationAuthor) -> typing.AsyncContextManager[IFileEditorSession]:
