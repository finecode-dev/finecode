...
from finecode.extension_runner.interfaces import ifilemanager, icache, ilogger
...
class MypySingleAstProvider(iast_provider.IMypySingleAstProvider):
    ...
    def __init__(self, file_manager: ifilemanager.IFileManager, cache: icache.ICache, logger: ilogger.ILogger):
        self.cache = cache
    ...
    async def get_file_ast(self, file_path: Path) -> mypy_nodes.MypyFile:
        try:
            cached_value = self.cache.get_file_cache(file_path=file_path, key=self.CACHE_KEY)
        ...
        module_program_path = self.get_file_program_path(file_path=file_path, root_package_parent_dir_path=base_dir)
        ...
        await self.cache.save_file_cache(file_path=file_path, file_version=file_version, key=self.CACHE_KEY, value=mypy_single_ast)
    ...
    def get_file_program_path(self, file_path: Path, root_package_parent_dir_path: Path) -> str:
